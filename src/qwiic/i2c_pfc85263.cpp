...
//    printf("\n");
    ...
    i2c_write_blocking(qwiic_i2c, pfc85263_addr, &buf[0], 11, false);

    buf[0] = RTCC_STOP;         // Write from the Stop register
    buf[1] = 0x00;              // Write 0 to the STOP register (Start the Clock)
    i2c_write_blocking(qwiic_i2c, pfc85263_addr, &buf[0], 2, false);
...
bool rtc_i2c_getdatetime()
{
  ...
  i2c_write_blocking(qwiic_i2c, pfc85263_addr, &buf[0], 1, false);    // Send the register number to read from
  uint8_t readbytes=i2c_read_blocking (qwiic_i2c, pfc85263_addr, &buf[0], 7, false);
  ...
  if (buf[0]&0x80)
   {
    ...
    rtc_i2c_setdatetime();
    return true;
   } else PM_INFO("[RTC] Success : Usr RTC Read Date/Time\n");
