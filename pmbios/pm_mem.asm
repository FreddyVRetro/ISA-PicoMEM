...
; PMCFG_PC_MMAP : Table generated by the BIOS / Hardware detection
...
;EOk DB 'Seg E Ok',0
;PSKO DB 'PSRAM Ko',0
;STR_CheckEMS DB '!!Check EMS !!',0x0D,0x0A,0

; Check for the segment D and E
; If none are Free, "disable" EMS
PM_Check_EMS:

;	printstr_w STR_CheckEMS
...
PM_Check_EMS_PSRAMOK:
	...
	CMP BH,1
	JE EMS_Is_Possible
	MOV byte CS:[PMCFG_EMS_Port],0	; Disable EMS (Port=0) (PM_Check)
EMS_Is_Possible:
...
MEM_Check_MEM_Changed:
	...
	XOR BX,BX
    CMP byte CS:[BV_Tandy],1		; Tandy : Start change Check at segment A
...
MEM_Copy_PCtoPM:
	...
	PUSH CS
	POP ES
	MOV SI,PMCFG_PC_MMAP   ; DS:SI > Detected PC Memory Map
	MOV DI,PMCFG_PM_MMAP   ; ES:DI > PicoMEM Memory Map
	MOV CX,32
...
; ** Fast RAM Test (16Kb Bloc) Check if there is RAM at 4 Different address
...
Test16KFast:
	...
	CLI					    ; Test : avoid IRQ during RAM Test	
Test16KFastLoop:
...
; ** TestRAMSlow: Check if there is RAM from the DS:SI Address
; Input  : DS:SI
;          CX : RAM Size in Kb
; Output : C Set if Failure
;          ! Change DS
...
TestRAMSlowLoop:
	...
	CMP word DS:[SI],0xAAAA
	JNE Test16KFail
    MOV word DS:[SI],0x5555
	CMP word DS:[SI],0x5555
	JNE Test16KFail
    MOV DS:[SI],AX			; Restore the value
	ADD SI,512              ; Test CX x 512 Bytes
	LOOP TestRAMSlowLoop
...
; 2- Add the EMS Memory pages
	...
	MOV BX,PMCFG_PM_MMAP+SEG_D		; BX: Segment D table offset
	CMP byte [PMCFG_EMS_Addr],0
	JE EMS_SegIsD
    ADD BX,4                        ; BX: Segment D table offset
EMS_SegIsD:
	MOV AX,MEM_EMS+256*MEM_EMS
    MOV word [BX],AX				; Write 4x the EMS Id
	MOV word [BX+2],AX
MEM_Init_NoEMS:

; Search for the memory Bottom
	MOV CX,64					; Search in all the RAM (1Mb)
	MOV SI,PMCFG_PM_MMAP
	XOR AX,AX

	CMP byte CS:[BV_Tandy],1	; Test if Tandy : Ram extention begining at segment A
	JNE MEM_SearchBottomLoop
    ADD SI,SEG_A	            ; Start bottom search at segment A
	MOV CX,6*4                  ; 6 segment remaining
	MOV AH,SEG_A
...
MEM_SearchBottom_VideoIgnore:	
	...
	JMP MEM_NoMEMAvailable  	; Everything is full
MEM_Bottom:

; 3- Add PMCFG_MAX_PMRAM*16KB with Pico Internal RAM (PMRAM)
	CMP byte [PMCFG_PMRAM_Ext],0
	JE MEM_Init_NoPMRAM

; Get the available PMRAM 
	XOR CX,CX
    MOV CL,byte [PMCFG_MAX_PMRAM]	; Sent by the Pico, via Cmakefile.ini
	CMP CL,0
	JE MEM_Init_NoPMRAM
	CMP CL,16						; can't be > 256Kb
	JA MEM_Init_NoPMRAM				; Avoid invalid value

MEM_FillPMRAM_Loop:

    PUSH SI
	CMP AL,0					; AL is the current location RAM Type
...
FillPMRAM_NoSkipVideo:
    ...
    MOV byte [SI-1],MEM_RAM		; Add the PMRAM
MEM_FillPMRAM_Skip:
    POP SI
	LODSB
...
FillPSRAM_NoSkipVideo:
    ...
    MOV byte [SI-1],MEM_PSRAM	; Add the PSRAM
MEM_FillPSRAM_Skip:	
	POP SI
	LODSB
	CMP SI,PMCFG_PM_MMAP+0x40
...
MEM_Init_NoPSRAM:
MEM_NoMEMAvailable:  ; Should "never" happens to have no free Memory zone
...
Count_ConvDoLoop:	
    ...
    CMP byte CS:[BV_TdyRAM],0  ; Tandy : Display added Conv RAM
	JE Count_Conv_NoTandy
	MOV BL,CS:[BV_TdyRAM]
Count_Conv_NoTandy:
